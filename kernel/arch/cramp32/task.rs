...
struct ExceptionStack {
    stack: [[u32; STACK_COUNT]; config::NUM_TASKS],
}

static mut EXCEPTION_STACKS: ExceptionStack = ExceptionStack {
    stack: [[0; STACK_COUNT]; config::NUM_TASKS],
};
...
fn init_stack(noarch_task: &NoarchTask, pc: usize) -> usize {
    unsafe {
        ...
        mmio::writev(sp, cramp32_start_task as u32); // ra
        for i in 0..7 {
            // gp, tp, s0-s11
            mmio::writev(sp.add(i * 2 + 1), 0);
            mmio::writev(sp.add(i * 2 + 2), 0);
        }
        mmio::writev(sp.add(15), pc as u32); // mepc
...
impl KArchTask for Cramp32Task {
    ...
    fn arch_task_switch(prev: &mut Cramp32Task, next: &Cramp32Task) {
        extern "C" {
